    ...
    <div class="card-body pb-0">
        <form action="{{route('video.update', ['video' => $video->id])}}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control form-control-sm @error('nama') is-invalid @enderror" type="text" id="nama" name="nama" value="{{old('nama') ?? $video->nama}}">
                @error('nama')
            ...
            <div class="form-group">
                ...
                <select class="form-control form-control-sm @error('status') is-invalid @enderror" id="status" name="status">
                    <option value=''></option>
                    <option value="0" @if(old('status', $video->status)=='0' ) selected @endif>Tidak Aktif</option>
                    <option value="1" @if(old('status', $video->status)=='1' ) selected @endif>Aktif</option>
                </select>
                @error('status')
                <div class="invalid-feedback mt-0">
                    <small>{{ $message }}</small>
